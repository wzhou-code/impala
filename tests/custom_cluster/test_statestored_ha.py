...
class TestStatestoredHA(CustomClusterTestSuite):
  ...
  def test_statestore_failover_query_resilience(self):
    ...
    try:
      ...
      while (time.time() - start_time < self.SS_PEER_TIMEOUT_S +
             self.RECOVERY_GRACE_PERIOD_S * 2):
        query_state = client.get_state(handle)
        if query_state == QueryState.EXCEPTION:
          break
        time.sleep(0.05)
      try:
        assert query_state == QueryState.EXCEPTION, "Query expected to be canceled"
